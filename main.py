    ...
    if choice == "Drugs":
        ...
        if choice == "Add":
            ...
            with col1:
                ...
                drug_mainuse = st.text_area("When to Use and where to buy")
            with col2:
...
def customer(username, password):
    if getauthenicate(username, password):
        ...
        for i, drug_info in enumerate(drug_result):
            st.subheader("Drug: " + drug_info[0])
            quantity = st.text_input(label="Quantity", key=i)
            st.info("When to USE: " + str(drug_info[2]))
        ...
        if st.button(label="Buy now"):
            ...
            for i, drug_info in enumerate(drug_result):
                if quantity.isdigit() and int(quantity) > 0:
                    order_items += drug_info[0] + ","
                    order_qty += str(quantity) + ","

            order_id = username + "#O" + str(random.randint(0, 1000000))
            order_add_data(username, order_items, order_qty, order_id)
